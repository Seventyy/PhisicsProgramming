...
namespace GXPEngine
{
	...
	public abstract class GameObject : Transformable
	{
		...
		/// </summary>
		virtual public float TimeOfImpact (GameObject other, float vx, float vy, out Core.Vector2 normal) {
			normal = new Core.Vector2 ();
            ...
            Core.Vector2 worldVelocity= parent.TransformDirection(vx, vy);
		...
		/// </param>
		public override Core.Vector2 TransformPoint(float x, float y) {
            Core.Vector2 ret = base.TransformPoint (x, y);
			if (parent == null) {
				return ret;
			} else {
		...
		/// </param>
		public Core.Vector2 TransformPoint(float x, float y, GameObject targetParentSpace) {
		...
		/// The y coordinate to transform.
		/// </param>
		public override Core.Vector2 TransformDirection(float x, float y) {
            Core.Vector2 ret = base.TransformDirection (x, y);
			if (parent == null) {
				return ret;
			} else {
		...
		/// The y coordinate to transform.
		/// </param>
		public override Core.Vector2 InverseTransformPoint(float x, float y) {
		...
		/// </param>
		public Core.Vector2 InverseTransformPoint(float x, float y, GameObject fromParentSpace) {
			if (parent == null || parent==fromParentSpace) {
				return base.InverseTransformPoint(x, y);
			} else {
                Core.Vector2 ret = parent.InverseTransformPoint(x, y, fromParentSpace);
		...
		/// The y coordinate to transform.
		/// </param>
		public override Core.Vector2 InverseTransformDirection(float x, float y) {
			if (parent == null) {
				return base.InverseTransformDirection(x, y);
			} else {
                Core.Vector2 ret = parent.InverseTransformDirection(x, y);
